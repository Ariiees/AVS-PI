...
#!/usr/bin/env python3
"""
Benchmark avg CPU & memory for AVS subscriber nodes (image/lidar/gps).
Small addition: optional listing of SSD global.sqlite3 trip rows (--list-trips).
"""
import argparse
import os
import signal
...
def list_ssd_trips(dbpath: Path):
    if not dbpath.exists():
        print(f"[INFO] no SSD global DB at {dbpath}")
        return
    try:
        conn = sqlite3.connect(str(dbpath))
        cur = conn.cursor()
        cur.execute("SELECT sensor_topic, day, trip_id, start_ts_ns, end_ts_ns FROM global ORDER BY sensor_topic, day, trip_id;")
        rows = cur.fetchall()
        if not rows:
            print("[INFO] SSD global DB has no rows")
            conn.close()
            return
        print("\nSSD global.sqlite3 rows:")
        for r in rows:
            print(f"  topic={r[0]} day={r[1]} trip={r[2]} start={r[3]} end={r[4]}")
        conn.close()
    except Exception as e:
        print(f"[ERR] reading SSD DB: {e}")
...
def main():
    ap = argparse.ArgumentParser(description="Benchmark avg CPU & memory for AVS subscribers.")
    ap.add_argument("--duration", type=float, default=30.0)
    ap.add_argument("--interval", type=float, default=1.0)
    ap.add_argument("--attach", action="store_true")
    ap.add_argument("--csv", type=Path, default=None)
    ap.add_argument("--namespace", default="")
    ap.add_argument("--list-trips", action="store_true", help="Print SSD global.sqlite3 rows and exit")
    args = ap.parse_args()

    if args.list_trips:
        list_ssd_trips(SSD_ROOT / "global.sqlite3")
        return
